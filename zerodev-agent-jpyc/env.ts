...
export function loadConfig(): RfcConfig {
	...
	return {
		...
		jpycDecimals,
		ownerPrivateKey: requiredHex32("OWNER_PRIVATE_KEY"),
...
/** Derive the owner + session viem accounts from the config keys. */
export function accountsFromConfig(cfg: RfcConfig) {
	return {
		owner: privateKeyToAccount(cfg.ownerPrivateKey),
		session: privateKeyToAccount(cfg.sessionPrivateKey),
	};
}
